...
<br>
	...
	$query="SELECT * FROM `posts` WHERE user_id='".$_SESSION["S_id"]."' ORDER BY `posts`.`time` DESC;";
			...
			while ($row=mysqli_fetch_row($result))
			{
				...
				echo ' 	<style>
					...
					#posts .post button:nth-of-type(1)
					{
						left:0%!important;
					}
					#posts .post button:nth-of-type(2)
					{
						left:33%!important;
					}
					#posts .post button:nth-of-type(3)
					{
						left:66%!important;
					}
				</style>
		...
		<form class="post the_post" >
			<img src="cloud/'.$_SESSION["S_id"].'/profile.jpg?v='.$_SESSION["my_version"].'">
				
			<a href="?PG=S_profile&S_user_id='.$_SESSION["S_id"].'"><h2 style="color:steelblue!important;">'.$_SESSION["S_username"].'</h2></a>
			...
			<p style="margin-left:5%; padding-bottom:30px;">'.$row[1].'</p>
				...
				if(file_exists('cloud/'.$_SESSION["S_id"].'/pro_post_img/'.$row[0].'.jpg'))
				{
					echo '<div class="post_img"><img src="cloud/'.$_SESSION["S_id"].'/pro_post_img/'.$row[0].'.jpg"></div>';
				}	

...
<?php
	...
	$query="SELECT users.* ,follow.time FROM users INNER JOIN follow ON follow.followed_user_id=users.id WHERE follow.user_id=".$_SESSION["id"]." ORDER BY `follow`.`time` DESC;";
		...
		<button style="cursor: pointer;" onClick="window.location=\'?PG=messages&S_user_id='.$row[0].'\';"><span style="color:purple!important;">message </span><i style="color:purple!important;" class="fas fa-envelope-open"></i></button>
			...
			$new_result=mysqli_query($connect,$new_query);
				...
				if(mysqli_num_rows($new_result) <= 0)
				{
					echo'
						...
						<button class="second" type="submit" style="cursor: pointer;" ><span style="color:green!important;" >follow <i style="color:green!important;" class="fas fa-user-plus"></i></span></button>
				...
				else
				{
					echo'
						...
						<button class="second" type="submit" style="cursor: pointer;" ><span style="color:green!important;" >friend <i style="color:green!important;" class="fas fa-user-friends"></i></span></button>
